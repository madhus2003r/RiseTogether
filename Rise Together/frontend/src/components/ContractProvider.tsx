...
export const ContractProvider: React.FC<ContractProviderProps> = ({ children }: ContractProviderProps) => {
	...
	const [crowdFundindContract, setCrowdFundindContract] = useState<GetContractReturnType>({} as GetContractReturnType)
	...
	useEffect(() => {
		if (walletClient && publicClient) {
			setCrowdFundindContract(
				getContract({
					address: CONTRACTS.LOCALHOST.CROWD_FUNDING.ADDRESS,
					abi: CONTRACTS.LOCALHOST.CROWD_FUNDING.ABI,
					publicClient,
					walletClient,
				}),
			)
